...
unsigned int lastPinState, preambles;
...
void myInterrupt (void)
{
	...
	//if (tsl_micros>=4750 && tsl_micros<5350) {

	if (tsl_micros>4900 && tsl_micros<=5300) {
		if (currentPinState)
		{
			start_pulse_counter++;
			printf("Time Since Last: %d, Time Since Start: %ds, Pin:%d, Pulse Count:%d\n",tsl_micros, millis()/1000,currentPinState,start_pulse_counter);
	...
	else if (tsl_micros>200 && tsl_micros<400 && start_pulse_counter>0)
	{
		if (tsl_micros>=200 && tsl_micros<=260)
		{
			printf("US: %d\n",tsl_micros);
		}
		else
		{
			printf("#####Expected 250us but got %dus\n",tsl_micros);
			start_pulse_counter=0;
		}
	}
	else
	...
	/*
		...
		if ((time_since_last==5 && !currentPinState) || 
                   ((tsl_micros>=200 && tsl_micros<=300) && currentPinState))
                {
                        start_pulse_counter++;
